...
class MonogenicNet(nn.Module):
    def __init__(self, monogenic_kwargs, Net, net_kwargs):
        super(MonogenicNet, self).__init__()
        self.monogenic = Monogenic(**monogenic_kwargs)
        
        if monogenic_kwargs['return_rgb'] == True or monogenic_kwargs['return_hsv'] == True:
            input_channels = monogenic_kwargs['nscale'] * 6
        elif monogenic_kwargs['return_phase_orientation'] == True:
            input_channels = monogenic_kwargs['nscale'] * 2
        else:
            input_channels = monogenic_kwargs['nscale']
        
        if monogenic_kwargs['return_input'] == True:
            input_channels += 1
        
        net_kwargs["input_channels"] = input_channels
